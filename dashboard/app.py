...
5. schedule.days — הנחיה רכה, לא חוקה. הימים המוגדרים הם ימים מועדפים אבל אינך חייב להגביל את עצמך אליהם. **קח יוזמה:**
   - אם schedule.<section>.days הוא רשימה ריקה ([]) — דלג לחלוטין (סקציה כבויה במכוון).
   - אם hebrew_day_num *כן* ברשימה — הפק תוכן רגיל.
   - אם hebrew_day_num *לא* ברשימה אבל הסקציה לא ריקה — אתה רשאי להציע תוכן לאותו יום אם זה הגיוני (למשל יום שבת ואין שום תוכן מתוכנן). סמן ב-notes_for_admin שמדובר בהצעה מחוץ לימים הרגילים, וב-needs_review=true בשורות שאתה לא בטוח לגביהן. הימנע אם השבוע כבר עמוס.
   - לטריוויה ואמוג'י: הצע batch אחד בלבד ביום, גם אם today אינו ביום מתוזמן רגיל — בהנחה שאין batch אחר היום (idempotence עדיין נכון).
   - דווח על כל הצעה מחוץ ללו"ז ב-notes_for_admin בנפרד תחת **"הצעות יוזמה"** כדי שהמנהל יוכל לאשר/לדחות.
